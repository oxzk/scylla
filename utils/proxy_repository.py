...
class ProxyRepository:
    async def add_proxy(self, proxy: ProxyData) -> Optional[int]:
        try:
            query = """
                INSERT INTO proxies (ip, port, protocol, country, anonymity, source, speed, is_valid)
    ...
    async def add_proxies_batch(self, proxies: List[ProxyData]) -> int:
        added_count = 0
    ...
    async def update_proxy_validation(self, proxy_id: int, is_valid: bool, 
        ...
        query = """
            ...
            WHERE id = $5
        """
        await db.execute(query, is_valid, speed, failure_count, datetime.now(), proxy_id)
    ...
    async def get_valid_proxies(self, protocol: Optional[str] = None, 
                               country: Optional[str] = None,
                               limit: int = 100) -> List[ProxyData]:
    ...
    async def get_proxies_for_validation(self, limit: int = 100) -> List[ProxyData]:
        query = """
            SELECT id, ip, port, protocol, country, anonymity, source, 
                   speed, is_valid, last_checked, failure_count, 
                   created_at, updated_at
